...
public class AmmendPlayerDeath : MonoBehaviour
{
    ...
    SerializedGameData[] newDataList;
    ...
    void Start()
    {
        try
        {
            ...
            // Split using regex to handle newlines or spaces between }{
            ...
            for (int i = 0; i < jsonObjects.Length; i++)
            {
                ...
                // Add braces back if split
                if (i != 0) obj = "{" + obj;
                if (i != jsonObjects.Length - 1) obj = obj + "}";

                // Deserialize individual object
        ...
        newDataList = correctLogs.ToArray();

        Debug.Log("corrected " + newDataList.Length + " log entries successfully!");

        newData.logs = newDataList;

        try
        {
            string json = JsonUtility.ToJson(newData, true);

            File.AppendAllText(saveFilePath, json);
            Debug.Log("Players saved to" + saveFilePath);
        }
        catch (Exception ex)
        {
